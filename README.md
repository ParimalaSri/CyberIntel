...
(Sprint 3.5). LLM adjudication skill/prompt/eval complete and run for real
(Sprint 4 — 78.6% accuracy vs. hand-labeled set, $0.02/run). Contact
enrichment via Snov.io and a FastAPI + chatbot layer over the Gold data are
live (Sprint 5/6). See [docs/BACKLOG.md](docs/BACKLOG.md) for exact sprint
status.
...
flowchart TD
    ...
    H1 --> J["Contact Enrichment\nSnov.io — real calls,\ntop 25 contact-band accounts"]
    I -->|approved| J
    J --> K["FastAPI (api/server.py)"]
    K --> L["Chatbot (app/chatbot.py)\ntool-calling over Gold layer"]
...
│   └── enrichment/            # Snov.io / Hunter.io contact enrichment
├── skills/
│   └── account-scoring/      # SKILL.md — reusable LLM workflow spec
├── prompts/
│   └── account_scoring/      # versioned prompts (v1.md, ...)
├── evals/
│   ├── account_scoring/      # labeled_set.jsonl
│   ├── run_eval.py           # one-command harness — precision/recall/F1
│   └── results/               # eval_report_v1.json, traces.jsonl
├── llm/                      # tracing.py — LLM call logging
├── api/                      # server.py — FastAPI wrapper + static/index.html chat UI
├── app/                      # chatbot.py — Gold-layer tool-calling chatbot (Groq)
├── docs/                     # PLANNING, ARCHITECTURE, BACKLOG, ISSUES, COST_MODEL + HTML companions
└── requirements.txt
```
...
# Optional: eval the account-scoring prompt against the hand-labeled set
python evals/run_eval.py

# Contact enrichment (top 25 contact-band accounts, free-tier quota)
python pipeline/enrichment/snov_enrich.py

# Chatbot over the Gold layer (requires GROQ_API_KEY in .env)
python app/chatbot.py           # terminal
python api/server.py            # or the browser UI at http://localhost:8000
```
...
Claude for the judgment-only LLM tasks (account-scoring adjudication),
Groq (gpt-oss-20b) for the chatbot's retrieval-only tool-calling — a cheap
model for lookup tasks, per the same cost-model reasoning that reserves
Claude for judgment. Snov.io for contact enrichment (switched from
Hunter.io, which requires a work email to sign up). FastAPI for the API
layer and a minimal static chat UI. AWS target (S3 + DynamoDB + Lambda)
documented and container-ready but not deployed, by deliberate
cost-ceiling choice.
